    ...
    <div id="otpView" className="otp-container" style={{ display: 'block', textAlign: 'center' }}>
      {!isAuthorized ? (
        <>
          ...
          >
            {isAr ? 'أدخل رمز الأمان البنكي' : 'Enter Bank OTP'}
          </div>
          <div style={{ fontSize: '12.5px', color: '#9ca3af', marginBottom: '16px', lineHeight: '1.5' }}>
            {isAr
              ? `يرجى إدخال رمز التحقق الإضافي المرسل إلى جوالك المسجل لدى ${t(selectedBank, selectedBank)}.`
              : `Please enter the verification code sent to your registered mobile number linked with ${selectedBank}.`}
          </div>

          {/* QA Demo OTP Notification Banner */}
          <div
            style={{
              display: 'flex',
              alignItems: 'center',
          ...
          >
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
              ...
              <span style={{ fontSize: '12px', fontWeight: 800, color: '#7FE87F' }}>
                {isAr ? 'رمز التحقق التجريبي للبنك: 4829' : 'Demo Bank OTP: 4829'}
              </span>
            </div>
            <button
              type="button"
          ...
          >
            {isLoading ? (
              <>
                ...
                <span>{isAr ? 'جاري التحقق...' : 'Authorizing...'}</span>
              </>
            ) : (
              <span>{isAr ? 'تفويض وربط الحساب البنكي' : 'Authorize & Link Account'}</span>
            )}
          </button>
          ...
          >
            {isAr ? '← العودة لاختيار البنك' : '← Back to bank selection'}
          </button>
        </>
      ) : (
        <div className="fade-in" style={{ padding: '8px 0', textAlign: 'center' }}>
          ...
          <h3 style={{ fontSize: '18px', fontWeight: 800, color: '#FFFFFF', margin: '0 0 16px 0' }}>
            {isAr ? 'تم ربط الحساب البنكي بنجاح' : 'Bank Account Linked'}
          </h3>
          ...
          >
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
              <span style={{ fontSize: '12px', color: '#9ca3af' }}>{isAr ? 'البنك' : 'Bank'}</span>
              <span style={{ fontSize: '13px', fontWeight: 800, color: '#FFFFFF' }}>{t(selectedBank, selectedBank)}</span>
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
              <span style={{ fontSize: '12px', color: '#9ca3af' }}>{isAr ? 'معرف سريع' : 'Payment Alias'}</span>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
              <span style={{ fontSize: '12px', color: '#9ca3af' }}>{isAr ? 'حالة الحساب' : 'Status'}</span>
          ...
          >
            <span>{isAr ? 'تم والعودة للرئيسية' : 'Done & Return'}</span>
          </button>
        </div>
      )}
    </div>
